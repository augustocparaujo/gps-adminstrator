...
while ($r = mysqli_fetch_array($sql1)) {
  echo '<option value="' . $r['id'] . '">' . $r['id'] . ' | Marca:' . $r['marca'] . ' | Modelo:' . $r['modelo'] . ' | Descrição:' . $r['descricao'] . ' | Estoque:' . $r['quantidade'] . ' | ' . Real($r['valorvenda']) . '</option>';
}
echo '
          </select>
        </label>
...
<script>
  ...
  function exibir(id) {
    $('#alterar').modal('show');
    $.get('clientes-produto-retorno.php', {
      id: id
    }, function(data) {
      //var ret = html(data);
  ...
  //caixar de valores
  $('#cartaocredito,#cartaodebito,#dinheiro,#pix,#taxa').keyup(function() {
    mostraTotal();
  });
  ...
  function mostraTotal() {
    ...
    if (cartaocredito != '') {
      valorCredito = cartaocredito;
    }
    if (cartaodebito != '') {
      valorDebito = cartaodebito;
    }
    if (dinheiro != '') {
    ...
    var totalReal = total.toLocaleString('pt-br', {
